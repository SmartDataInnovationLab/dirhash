 ...
 */
class FixedLengthBinaryRecordReader
  ...
  override def nextKeyValue(): Boolean = {
    ...
    if (currentPosition < splitEnd) {
      ...
      var done = 0
      breakable {
        while (done < recordLength) {
          val res = fileInputStream.read(buffer, done, recordLength - done)
          if (res < 0)
            break
          if (res == 0)
      ...
      if (done != recordLength) {
        recordValue = new BytesWritable(buffer, done)
      }
